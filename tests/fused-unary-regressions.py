#!/usr/bin/env python3
"""Compare unary fusion against the pair scheduler in a copied runtime.
...
def without_fusion(s):
    ...
    canonical="""                            rc_bounded_recent_turn ^= 1;
                            ...
                            work += 2;"""
    return s[:begin]+canonical+s[end:]
...
def main():
    ...
    reference.write_text(without_fusion(s))
    report={'status':'running','checks':[],'source_sha256':{},
            'reference_transformation':'Replace only fused continuation with the original two-unit pair body; retain the pair guard and locality scheduler.',
